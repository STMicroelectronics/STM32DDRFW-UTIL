...
HAL_StatusTypeDef HAL_UART_Transmit_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
  ...
  if (huart->gState == HAL_UART_STATE_READY)
  {
    ...
    /* The UART may not issue a new DMA request after disable then re-enable DMAT bit.
       So the reset of DMAT bit could impact an another UART transfer in DMA mode.
       A work around has been set up to avoid this limitation consist to disable then re-enable TE bit */
    uint32_t tickstart;
