...
static void I2C_MspDeInit(I2C_HandleTypeDef *hI2c)
{
  ...
  gpio_init_structure.Pin = DISCO_I2C_SCL_PIN;
  BSP_ENTER_CRITICAL_SECTION(DISCO_I2C_SCL_GPIO_PORT);
  HAL_GPIO_DeInit(DISCO_I2C_SCL_GPIO_PORT, gpio_init_structure.Pin);
  BSP_EXIT_CRITICAL_SECTION(DISCO_I2C_SCL_GPIO_PORT);
  gpio_init_structure.Pin = DISCO_I2C_SDA_PIN;
  BSP_ENTER_CRITICAL_SECTION(DISCO_I2C_SDA_GPIO_PORT);
  HAL_GPIO_DeInit(DISCO_I2C_SDA_GPIO_PORT, gpio_init_structure.Pin);
  BSP_EXIT_CRITICAL_SECTION(DISCO_I2C_SDA_GPIO_PORT);

  /* Disable GPIO clock */
  DISCO_I2C_SCL_GPIO_CLK_DISABLE();
  DISCO_I2C_SDA_GPIO_CLK_DISABLE();
  
  /* Disable I2C clock */
  DISCO_I2C_CLK_DISABLE();

  /* Disable NVIC for I2C */
#if defined(CORE_CA7)
  IRQ_Disable(DISCO_I2C_ER_IRQn);
  IRQ_Disable(DISCO_I2C_EV_IRQn);
#elif defined(CORE_CM4)
  HAL_NVIC_DisableIRQ(DISCO_I2C_ER_IRQn);
  HAL_NVIC_DisableIRQ(DISCO_I2C_EV_IRQn);
#endif
}
