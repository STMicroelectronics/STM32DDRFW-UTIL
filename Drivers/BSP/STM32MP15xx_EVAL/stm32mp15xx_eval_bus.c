...
{
  return I2C_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length);
}
...
{
  return I2C_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_8BIT, pData, Length);
}
...
{
  return I2C_WriteReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length);
}
...
{
  return I2C_ReadReg(DevAddr, Reg, I2C_MEMADD_SIZE_16BIT, pData, Length);
}
...
int32_t BSP_I2C_RegisterDefaultMspCallbacks (void)
{
  ...
  if(HAL_I2C_RegisterCallback(&hI2c, HAL_I2C_MSPINIT_CB_ID, I2C_MspInit) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else if(HAL_I2C_RegisterCallback(&hI2c, HAL_I2C_MSPDEINIT_CB_ID, I2C_MspDeInit) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else
...
int32_t BSP_I2C_RegisterMspCallbacks (BSP_I2C_Cb_t *Callback)
{
  ...
  if(HAL_I2C_RegisterCallback(&hI2c, HAL_I2C_MSPINIT_CB_ID, Callback->pMspI2cInitCb) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else if(HAL_I2C_RegisterCallback(&hI2c, HAL_I2C_MSPDEINIT_CB_ID, Callback->pMspI2cDeInitCb) != HAL_OK)
  {
    ret = BSP_ERROR_PERIPH_FAILURE;
  }
  else
...
static void I2C_MspInit(I2C_HandleTypeDef *hI2c)
{
  ...
  /*** Configure the GPIOs ***/
  /* Enable GPIO clock */
  EVAL_I2C_SCL_GPIO_CLK_ENABLE();
  EVAL_I2C_SDA_GPIO_CLK_ENABLE();

  /* Configure I2C Tx as alternate function */
  gpio_init_structure.Pin = EVAL_I2C_SCL_PIN;
  gpio_init_structure.Mode = GPIO_MODE_AF_OD;
  gpio_init_structure.Pull = GPIO_NOPULL;
  gpio_init_structure.Speed = GPIO_SPEED_FREQ_HIGH;
  gpio_init_structure.Alternate = EVAL_I2C_SCL_AF;
  BSP_ENTER_CRITICAL_SECTION(EVAL_I2C_SCL_GPIO_PORT);
  HAL_GPIO_Init(EVAL_I2C_SCL_GPIO_PORT, &gpio_init_structure);
  BSP_EXIT_CRITICAL_SECTION(EVAL_I2C_SCL_GPIO_PORT);

  /* Configure I2C Rx as alternate function */
  gpio_init_structure.Pin = EVAL_I2C_SDA_PIN;
  gpio_init_structure.Alternate = EVAL_I2C_SDA_AF;
  BSP_ENTER_CRITICAL_SECTION(EVAL_I2C_SDA_GPIO_PORT);
  HAL_GPIO_Init(EVAL_I2C_SDA_GPIO_PORT, &gpio_init_structure);
  BSP_EXIT_CRITICAL_SECTION(EVAL_I2C_SDA_GPIO_PORT);

  /* Enable I2C clock */
  EVAL_I2C_CLK_ENABLE();

  /* Force the I2C peripheral clock reset */
  EVAL_I2C_FORCE_RESET();

  /* Release the I2C peripheral clock reset */
  EVAL_I2C_RELEASE_RESET();

  /*** Configure the I2C peripheral clock ***/
#if (UTIL_PMIC_I2C_PORT == UTIL_I2C1) || (UTIL_PMIC_I2C_PORT == UTIL_I2C2)
...
#if defined(CORE_CA7)
  IRQ_SetPriority(EVAL_I2C_EV_IRQn, 0);
  IRQ_Enable(EVAL_I2C_EV_IRQn);
#elif defined(CORE_CM4)
  HAL_NVIC_SetPriority(EVAL_I2C_ER_IRQn, 0, 1);
  HAL_NVIC_EnableIRQ(EVAL_I2C_ER_IRQn);
  HAL_NVIC_SetPriority(EVAL_I2C_EV_IRQn, 0, 2);
  HAL_NVIC_EnableIRQ(EVAL_I2C_EV_IRQn);
#else
...
static void I2C_MspDeInit(I2C_HandleTypeDef *hI2c)
{
  ...
  gpio_init_structure.Pin = EVAL_I2C_SCL_PIN;
  BSP_ENTER_CRITICAL_SECTION(EVAL_I2C_SCL_GPIO_PORT);
  HAL_GPIO_DeInit(EVAL_I2C_SCL_GPIO_PORT, gpio_init_structure.Pin);
  BSP_EXIT_CRITICAL_SECTION(EVAL_I2C_SCL_GPIO_PORT);
  gpio_init_structure.Pin = EVAL_I2C_SDA_PIN;
  BSP_ENTER_CRITICAL_SECTION(EVAL_I2C_SDA_GPIO_PORT);
  HAL_GPIO_DeInit(EVAL_I2C_SDA_GPIO_PORT, gpio_init_structure.Pin);
  BSP_EXIT_CRITICAL_SECTION(EVAL_I2C_SDA_GPIO_PORT);

  /* Disable GPIO clock */
  EVAL_I2C_SCL_GPIO_CLK_DISABLE();
  EVAL_I2C_SDA_GPIO_CLK_DISABLE();

  /* Disable I2C clock */
  EVAL_I2C_CLK_DISABLE();

  /* Disable NVIC for I2C */
#if defined(CORE_CA7)
  IRQ_Disable(EVAL_I2C_ER_IRQn);
#elif defined(CORE_CM4)
  HAL_NVIC_DisableIRQ(EVAL_I2C_ER_IRQn);
  HAL_NVIC_DisableIRQ(EVAL_I2C_EV_IRQn);
#else
#error "No core specified !!!"
#endif
}
