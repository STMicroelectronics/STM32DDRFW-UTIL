                                             ...
                                             const RTC_PrivilegeStateTypeDef *privilegeState)
{
  ...
  /* TAMP, Monotonic counter and Backup registers privilege configuration
     Warning : privilegeState->backupRegisterPrivZone is only writable in secure mode or if trustzone is disabled.
               In non secure mode, a notification is generated through a flag/interrupt in
               the TZIC (TrustZone interrupt controller). The bits are not written. */
  WRITE_REG(TAMP->PRIVCR,
            (privilegeState->tampPrivilegeFull          |
             privilegeState->backupRegisterPrivZone     |
             privilegeState->MonotonicCounterPrivilege  |
             privilegeState->MonotonicCounter2Privilege));
...
HAL_StatusTypeDef HAL_RTCEx_CidModeSet(const RTC_HandleTypeDef *hrtc, const RTC_CidStateTypeDef *cidState)
{
  ...
  MODIFY_REG(TAMP->SECCFGR,
             (TAMP_SECCFGR_BKPRWSEC | TAMP_SECCFGR_BKPWSEC),
             ((cidState->backupRegisterStartZone2 << TAMP_SECCFGR_BKPRWSEC_Pos) |
              (cidState->backupRegisterStartZone3 << TAMP_SECCFGR_BKPWSEC_Pos)));
  ...
  WRITE_REG(TAMP->BKPRIFR3,
            ((cidState->backupRegisterStartRif0Zone3 << TAMP_BKPRIFR3_BKPRWRIF1_Pos) |
             (cidState->backupRegisterStartRif2Zone3 << TAMP_BKPRIFR3_BKPRWRIF2_Pos)));
